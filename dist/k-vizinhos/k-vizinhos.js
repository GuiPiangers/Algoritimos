...
function calculateUserProximity(user1, user2) {
...
function getNestBraderhood(userName) {
    const user = users[userName];
    const nestBraderhood = [];
    for (let key in users) {
        if (!(key === userName)) {
            const value = calculateUserProximity(user, users[key]);
            if (nestBraderhood.length <= 0 ||
                ...
                nestBraderhood.forEach((item, index) => {
                    if (value < item.value)
                        nestBraderhood.splice(index, 0, { value, ...users[key] });
                });
            }
        }
    }
...
//# sourceMappingURL=k-vizinhos.js.map